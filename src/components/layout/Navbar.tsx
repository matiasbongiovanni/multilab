              ...
              <AnimatePresence>
                {isServiciosOpen && (
                  <motion.div
                    ...
                    className="absolute top-full left-1/2 -translate-x-1/2 mt-3 w-72 bg-white/95 backdrop-blur-2xl rounded-2xl shadow-[0_20px_60px_-15px_rgba(46,125,50,0.15)] border border-green-100 py-2 z-50 overflow-hidden"
                    role="menu"
                  >
                    {/* Top glow */}
                    ...
                    {serviciosLinks.map((link) => (
                      <Link
                        ...
                        className={`flex items-center gap-3 px-5 py-3 text-sm transition-all duration-200 ${isActive(link.href)
                          ...
                          : "text-[#1a2e1a]/60 hover:text-[#2E7D32] hover:bg-green-50 hover:pl-7"
                          }`}
                      >
                        <span className={`w-1.5 h-1.5 rounded-full shrink-0 transition-colors ${isActive(link.href) ? "bg-[#2E7D32]" : "bg-[#1a2e1a]/20"
                          }`} />
                        {link.label}
                      </Link>
                    ))}
                    ...
                    {serviciosLinks.map((link) => (
                      <Link
                        ...
                        className={`flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm transition-all duration-200 ${isActive(link.href)
                          ? "text-[#2E7D32] bg-green-50 font-semibold"
                          : "text-[#1a2e1a]/50 hover:text-[#2E7D32] hover:bg-green-50"
                          }`}
                      >
